...
class MultiCohortDatasetNoHypnogram(Dataset):
    ...
    def __init__(self, config, subject=None, subset='test'):
        ...
        print("[cohorts and subsets]", cohorts, subsets)
        df = pd.concat(
            [pd.read_csv(os.path.join(self.data_dir, 'csv', cohort + '.csv'), index_col=0) for cohort
             in list(set(cohorts))], ignore_index=True)
        print("»» CSVs lidos", [os.path.join(self.data_dir, 'csv', cohort + '.csv') for cohort in list(set(cohorts))])
        ...
        print("»» DF 1", self.df.shape, "\n", self.df)

        # Prune for subsets in files
        list_files = os.listdir(os.path.join(self.data_dir, 'h5'))
        print("»» listfiles:", list_files)
        #self.df = self.df.loc[df.FileID.str.lower().isin([f[:-3] for f in list_files]), :] #original \
        ## !! MODIFICADO
        ...
        print("[ prints ]", self.train_fraction, self.subject, self.num_subjects, self.segment_length, )
