...
The architecture of the network used in \cite{Alexander2020} was inspired by ResNet-50 and adapted to 1-dimensional, time-dependent signals for classification tasks. A deep learning approach was used because it doesn't require preprocessing or manual feature extraction, and the mapping function is automaticly learned end to end.
...
\begin{itemize}
    ...
    \item \textbf{A feature extraction module}, that comprises $R$ residual blocks, each using 2D convolutions to reduce the increase again the number of feature maps, batch normalizations and activations. Each block is simple, ensuring good conversion, and adding a lot of blocks give more expression to the network.
...
\begin{itemize}
    \item \textbf{Setting up the .edf files} to be read in a folder and modify a config file to let the pipeline know the new location of the data. 
...
The first step of coding for this project was to fork the original GitHub repository \cite{Alexander2020} to keep all of the original content while allowing me to develop on top of that, keeping the coherence and structure. However some modifications had to be made on the code, and some envolved copying script files and hard coding some modifications on them, in order to fit some incompatibilities between the pipeline and our data and objectives for the project.
...
Because of that, when mapping the channels from our data to the channels in the pipeline, the chin EMG and and EOG were left black, meaning that, after preprocessing of the channels, 1 out of the 4 data values that was passed to the model for each timestamp was zeroed out. 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item A hypnogram comparison between which sleep stahe each rater (each pipeline) has predicted for each time step, visualized as all hypnograms' time series superimposed as a function of time.
        ...
        \item A matrix of inter-rater agreement (accuracy), essencially saying how much each pair of raters agree with each other.
...
Aditionally, the prediction model could be swapped by another as long as they take the same input and output. 
...
% COOPERATION + GOAL~~~
Finnaly, this project was done with a lot of colaboration from a collegue, who assessed the quality of another automatic sleep staging pipeline, using sleepEEGpy \cite{belonosov2023sleepeegpy}, a wrap around of another sleep staging python library: Yasa \cite{2021yasa}. The multi-rater analysis part of this pipeline was mostly used to compare the results from each of the 2 pipelines, and because labeled sleep EEG data was unavailable, agreement between the raters was considered as a good enough proxi for the ground truth. In fact, as one the goal of the project was to expand Optoceutics' library of labeled sleep EEG data, and to assess the feasibility of further sleep studies, this multi-rater setup was more than enough.
